...
public class TutorialView extends GLSurfaceView implements GestureDetector.OnDoubleTapListener, GestureDetector.OnGestureListener {
    ...
    public static final int TUTORIAL_CHANGE_MASK_OPERATION_CMD = 0;
    public static final int TUTORIAL_CHANGE_FIRST_MASK_CMD = 1;
    public static final int TUTORIAL_CHANGE_SECOND_MASK_CMD = 2;
    public static final int TUTORIAL_ABOUT_CMD = 3;
    ...
    private String maskOperationStr(int maskOperation) {
    ...
    private void activityTitleUpdate() {
        ...
        ((Activity)activity).setTitle(appName + " (" + maskOperationStr(tutorial.getMaskOperation()) + ")");
    }
    ...
    *****************************************************************/
    ...
    private void tutorialInit() {
        ...
        byte[] rawStarData = null;
        byte[] rawCloudData = null;

        // read images
        try {
            InputStream rawStarStream = getResources().openRawResource(R.raw.star);
            rawStarData = toByteArray(rawStarStream);
        }
        catch (java.io.IOException e) {
            System.err.println("Star image file error, read failed.\n" + e);
        }

        try {
            InputStream rawCloudStream = getResources().openRawResource(R.raw.cloud);
            rawCloudData = toByteArray(rawCloudStream);
        }
        catch (java.io.IOException e) {
            System.err.println("Cloud image file error, read failed.\n" + e);
        }

        tutorial = new Tutorial(vg);
        // init tutorial application
        tutorial.init(openvgSurfaceWidthGet(), openvgSurfaceHeightGet(), rawStarData, rawCloudData);
    ...
    boolean tutorialMenuOption(int option) {
        ...
        switch (option) {
            ...
            case TUTORIAL_CHANGE_MASK_OPERATION_CMD:
                tutorial.toggleMaskOperation();
                activityTitleUpdate();
                break;

            case TUTORIAL_CHANGE_FIRST_MASK_CMD:
                tutorial.toggleMask1();
                break;

            case TUTORIAL_CHANGE_SECOND_MASK_CMD:
                tutorial.toggleMask2();
                break;
