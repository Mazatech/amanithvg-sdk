...
#define WINDOW_TITLE "AmanithVG Tutorial 10 - Press F1 for help"
...
static const char* maskOperationStr(const VGMaskOperation maskOperation) {

    static const char* maskOperationStrMap[] = {
        "CLEAR",
        "FILL",
        "SET",
        "UNION",
    ...
    return ((maskOperation >= VG_CLEAR_MASK) && (maskOperation <= VG_SUBTRACT_MASK)) ? maskOperationStrMap[maskOperation - VG_CLEAR_MASK] : maskOperationStrMap[6];
}
...
static void windowTitleUpdate(void) {
    ...
    if (time1 - time0 > 1000) {
        ...
        sprintf(title, "(%d fps, %s) "WINDOW_TITLE, (VGint)fps, maskOperationStr(getMaskOperation()));
...
static void helpDialog(void) {
    ...
    strcat(msg, "F1: Help.\n");
    strcat(msg, "Mouse: Move alpha masks.\n");
    strcat(msg, "M: Change mask operation.\n");
    strcat(msg, "F: Change first mask primitive.\n");
    strcat(msg, "G: Change second mask primitive.\n");
