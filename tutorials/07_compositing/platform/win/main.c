...
#define WINDOW_TITLE "AmanithVG Tutorial 07 - Press F1 for help"
...
static const char* blendModeStr(const VGint blendMode) {
    ...
    if ((blendMode >= VG_BLEND_CLEAR_MZT) && (blendMode <= VG_BLEND_EXCLUSION_MZT)) {
        return blendModeStrMap[blendMode - VG_BLEND_CLEAR_MZT + 10];
    }
    else {
...
static void windowTitleUpdate(void) {
    ...
    if (time1 - time0 > 1000) {
        ...
        sprintf(title, "(%d fps, %s) "WINDOW_TITLE, (VGint)fps, blendModeStr(getBlendMode()));
...
static void helpDialog(void) {
    ...
    strcat(msg, "F1: Help.\n");
    strcat(msg, "Mouse: Move images.\n");
    strcat(msg, "B: Change blend mode.\n");
...
static VGboolean processKeysPressure(void) {
    ...
    // B
    if ((keysPressed['b']) || (keysPressed['B'])) {
        keysPressed['b'] = VG_FALSE;
        keysPressed['B'] = VG_FALSE;
        toggleBlendMode();
    }
